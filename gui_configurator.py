...
class WorkflowConfiguratorApp:
    ...
    def toggle_scheduler(self):
        ...
        if status in ['running', 'timeout']:
            if messagebox.askyesno("Conferma", "Sei sicuro di voler fermare lo scheduler?"):
                try:
                    # Invia SIGTERM per una chiusura pulita
                    os.kill(pid, signal.SIGTERM)
                    logging.info(f"Segnale di terminazione inviato allo scheduler (PID: {pid}).")
                except (ProcessLookupError, PermissionError) as e:
                    logging.error(f"Impossibile terminare il processo {pid}: {e}. Potrebbe essere già terminato.")
                # Pulisce il file di stato in caso di timeout
