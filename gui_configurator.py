...
class WorkflowConfiguratorApp:
    ...
    def save_workflows(self):
        # Prima di salvare, aggiorna il flusso correntemente visualizzato con i dati della UI
    ...
    def create_widgets(self):
        ...
        # --- Pannello Sinistro (Elenco Flussi) ---
    ...
    def on_workflow_select(self, event):
        ...
        # Prima di cambiare, salva lo stato corrente della UI nel dizionario
        if self.selected_workflow_name:
            self.update_workflow_from_ui(self.selected_workflow_name)
    ...
    def update_workflow_from_ui(self, flow_name):
        ...
        if not new_flow_name:
            # Non aggiornare se il nome è vuoto, ma mantieni i dati
            return
        ...
        if new_flow_name != flow_name:
            ...
            self.selected_workflow_name = new_flow_name # Aggiorna il nome selezionato
        else:
    ...
    def add_new_workflow(self):
        ...
        i = 1
        while f"Nuovo Flusso {i}" in self.workflows:
            i += 1
        new_name = f"Nuovo Flusso {i}"

        self.workflows[new_name] = {
            "tasks": [], "schedule_time": "09:00", "schedule_days": []
        }
        self.populate_workflows_list()
        # Seleziona il nuovo flusso
    ...
    def add_task(self):
        filepaths = filedialog.askopenfilenames(
            title="Seleziona script Python",
            filetypes=[("Python files", "*.py"), ("All files", "*.*")]
        )
        for filepath in filepaths:
            # Salva il percorso relativo se possibile per portabilità
            try:
                rel_path = os.path.relpath(filepath)
                self.tasks_listbox.insert(tk.END, rel_path)
            except ValueError:
                self.tasks_listbox.insert(tk.END, filepath) # Fallback per dischi diversi (Windows)

    def remove_task(self):
        selected_indices = self.tasks_listbox.curselection()
        if not selected_indices:
            return
        # Rimuovi partendo dal fondo per non invalidare gli indici
    ...
    def move_task_down(self):
        ...
        # Itera in ordine inverso per gestire selezioni multiple
        for i in sorted(selected_indices, reverse=True):
...
if __name__ == "__main__":
    ...
    # Gestione della chiusura: salva prima di uscire
